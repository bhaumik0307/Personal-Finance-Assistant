...
const NoDataState = () => (
  <div className="bg-gray-50 border border-gray-200 rounded-lg p-12 text-center">
    ...
    <p className="text-gray-500">Add some transactions to see your financial reports</p>
  </div>
)

const Reports = () => {
  const { user, loading: userLoading, fetchCurrentUser } = useAuthStore()
  const { transactions, loading, error, fetchUserTransactions, resetMessages } = useTransactionStore()
  ...
  const handleRefresh = useCallback(() => {
    if (user?._id || user?.id) {
      ...
      if (dateRange === "custom" && customStart && customEnd) {
        fetchUserTransactions(userId, customStart, customEnd)
      } else if (dateRange !== "custom") {
        const { start, end } = getDateRange(dateRange)
        fetchUserTransactions(userId, start, end)
      }
    }
  }, [user, dateRange, customStart, customEnd, fetchUserTransactions])
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Summary Cards */}
      <ReportsSummaryCards stats={stats} formatCurrency={formatCurrency} />

      {/* Charts Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <ExpensesPieChart data={expensesByCategory} formatCurrency={formatCurrency} />
        <MonthlyTrendsChart data={monthlyTrends} formatCurrency={formatCurrency} />
      </div>

      {/* Daily Expenses and Income vs Expenses */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <DailyExpensesChart data={dailyExpenses} formatCurrency={formatCurrency} />
        <IncomeVsExpensesChart data={monthlyTrends} formatCurrency={formatCurrency} />
      </div>

      {/* Category Breakdown Table */}
      <CategoryBreakdownTable data={categoryBreakdown} stats={stats} formatCurrency={formatCurrency} />
    </div>
  )
}
